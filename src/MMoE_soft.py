...
class MMoE_SOFT(MultiTaskModel):
    def __init__(self,
        ...
        projection_hidden = max(64, expert_hidden_units[-1] // 2)
        projection_out = max(32, projection_hidden // 2)
        self.z_layers_0 = torch.nn.Linear(expert_hidden_units[-1], projection_hidden, bias=False)
        self.z_layers_1 = torch.nn.Linear(projection_hidden, projection_out, bias=False)
        self.batch_norm = torch.nn.BatchNorm1d(num_features=projection_out, affine=True)
